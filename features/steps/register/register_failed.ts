...
Then(/^I should see (.*) and (.*)$/, async function (error1, error2) {
    await expect(homepage.register.errorFirstname).toHaveTextContaining(error1)
    await expect(homepage.register.errorLastname).toHaveTextContaining(error2)
})